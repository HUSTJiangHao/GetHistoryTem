...
bool GetTemLib::getHistoryTemVec(float p[24], int Month, int Day, int LOCATION)
{
	...
	int Line = JD*24-23;//¥”csv ˝æ›÷–µ⁄Line––ø™ º»° ˝æ›
	//cout << "Line:" << Line << endl;
	...
	{
	case(HK_HAERBIN):
		FilePath = m_libPath + "/1π˛∂˚±ı.csv";
		break;
	case(HK_SHENYANG):
		FilePath = m_libPath + "/2…Ú—Ù.csv";
		break;
	case(HK_BEIJING):
		FilePath = m_libPath + "/3±±æ©.csv";
		break;
	case(HK_JINAN):
		FilePath = m_libPath + "/4º√ƒœ.csv";
		break;
	case(HK_HEFEI):
		FilePath = m_libPath + "/5∫œ∑ .csv";
		break;
	case(HK_CHANGSHA):
		FilePath = m_libPath + "/6≥§…≥.csv";
		break;
	case(HK_GUANGZHOU):
		FilePath = m_libPath + "/7π„÷›.csv";
		break;
	case(HK_HAIKOU):
		FilePath = m_libPath + "/8∫£ø⁄.csv";
		break;
	case(HK_DULAN):
		FilePath = m_libPath + "/9∂º¿º.csv";
		break;
	case(HK_WULUMUQI):
		FilePath = m_libPath + "/10Œ⁄¬≥ƒæ∆Î.csv";
		break;
...
bool GetTemLib::ReadCsvTem(const char * FilePath, float get[24], int firstLine)
{
	...
	string line;
	int lineNum = 0;//±Í÷æ∂¡µΩµ⁄º∏––
	int TimeNum = 0;// ˝◊Èœ¬±Í
	while(getline(file, line))
	{
		if (lineNum >= firstLine)//µΩ÷∏∂®Œª÷√ø™ ºœÚ ˝◊È¥Ê ˝æ›
		{
			istringstream sin(line);
			vector<string> fields; 
			string field;

			while (getline(sin, field, ',')) //Ω´◊÷∑˚¥Æ¡˜sin÷–µƒ◊÷∑˚∂¡»ÎµΩfield◊÷∑˚¥Æ÷–£¨“‘∂∫∫≈Œ™∑÷∏Ù∑˚
			{

				fields.push_back(field); //Ω´∏’∏’∂¡»°µƒ◊÷∑˚¥ÆÃÌº”µΩœÚ¡øfields÷–

			}
			if (fields.size() != 8) // »± ßœÓ
			{
				get[TimeNum] = -273.0;
				TimeNum++;
			}
